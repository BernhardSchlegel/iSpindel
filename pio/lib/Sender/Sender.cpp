...
uint32_t SenderClass::sendBricks()
{
  ...
  if (_secureClient.connect("https://brewbricks.com", 443))
  {
    CONSOLELN(F("\nSender: Bricks posting"));

    String msg = F("POST /api/iot/v1");
    msg += F(" HTTP/1.1\r\nHost: ");
    msg += UBISERVER;
    msg += F("\r\nUser-Agent: ESP8266\r\nConnection: close\r\nContent-Type: application/json\r\nContent-Length: ");
    msg += measureJson(_doc);
    ...
    _secureClient.println(msg);
    serializeJson(_doc, _secureClient);
  ...
  int timeout = 0;
  while (!_secureClient.available() && timeout < CONNTIMEOUT)
  {
    timeout++;
    delay(1);
  }
